 ...
 * 
 * 除了处理stage的DAG，它还负责决定运行每个task的最佳位置，基于当前的缓存状态，将这些最佳位置提交给底层的
 * TaskSchedulerImpl。此外，它会会处理由于shuffle输出文件丢失导致的失败，在这种情况下，旧的stage可能就会被重新
      ...
      properties: Properties = null)
  {
    ...
    if (finalStage != null) {
      ...
      if (shouldRunLocally) {
        ...
        // 必须对stage划分算法很清晰，直到你自己编写的spark application被划分为了几个job
        // 每个job被划分成了几个stage
        // 每个stage，包括了你的哪些代码
