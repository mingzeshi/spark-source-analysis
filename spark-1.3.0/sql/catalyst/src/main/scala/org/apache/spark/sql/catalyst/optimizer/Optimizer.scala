...
object DefaultOptimizer extends Optimizer {
  ...
  // 这里的batches是非常非常重要的
  // 这里就封装了每一个Spark SQL版本中，可以对逻辑执行计划执行的优化策略
  ...
  // 让我们传递给Spark SQL执行的SQL语句，本身就已经是最优的
  // 这样的话，就可以避免在执行SQL解析的时候，进行大量的Spark SQL内部的优化
  // 那么，在某种程度上，也可以提升性能
  val batches =
    Batch("Combine Limits", FixedPoint(100),
      CombineLimits) ::  
      // CombineLimites，其实就是合并limit语句
    ...
    Batch("Filter Pushdown", FixedPoint(100),
      ...
      // 那么就用select x from table; 不要用select x from (select * from table)的查询语句
