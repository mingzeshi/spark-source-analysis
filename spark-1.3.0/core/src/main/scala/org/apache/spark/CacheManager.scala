...
private[spark] class CacheManager(blockManager: BlockManager) extends Logging {
    ...
    blockManager.get(key) match {
      ...
      case None =>
        ...
        try {
          ...
          // 所以读取了checkpoint数据，或者是重新计算数据之后，要用putInBlockManager方法
  ...
  private def putInBlockManager[T](
    ...
    // 如果指定了内存级别的话，就比较复杂了
      ...
      // 如果unrollSafelly()方法判断是数据可以写入内存，那么很好
      // 就将数据写入内存
      // 但是如果unrollSafelly()方法判断某些数据，无法写入内存，那么没有办法，只能写入磁盘文件
