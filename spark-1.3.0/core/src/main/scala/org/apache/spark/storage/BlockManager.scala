...
private[spark] class BlockManager(
  ...
  // 然后，然后呢，BlockInfo最大的作用，就是说，用于作为多线程并发访问同一个Block的同步监视器
  ...
  def initialize(appId: String): Unit = {
    // 不好意思，上一讲，讲BlockManager的架构的时候，犯了一个小错误
    // 因为我们这里都是针对spark 1.3.0版本的源码进行剖析，已经属于spark比较新的版本了
    ...
    // 使用到了哪些东西？executorId（每个BlockManager都关联一个Executor，BlockTransferService的hostname、
        ...
        if (level.useDisk) {
          ...
          if (!level.useMemory) {
            ...
            if (!level.deserialized || !asBlockResult) {
              ...
              // 那么从disk读取出来之后，其实会尝试将其放入MemeoryStore中，也就是缓存到内存中
