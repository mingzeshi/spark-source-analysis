...
abstract class RDD[T: ClassTag](
   ...
   * 那么首先执行到该rdd的iterator()之后，会先发现storageLevel != StorageLevel.NONE把
   * 那么就会通过CacheManager去获取数据，此时会发现通过BlockManager获取不到数据（因为是第一次执行么）
   * 那么就会第一次还是会计算一次该rdd的数据，然后通过CacheManager的putInBlockManager()将其通过BlockManager
   * 进行持久化
