...
class BlockManagerMasterActor(val isLocal: Boolean, conf: SparkConf, listenerBus: LiveListenerBus)
  ...
  // 这个map，映射了blcok manager id到block manager的info
  ...
  private def removeBlockManager(blockManagerId: BlockManagerId) {
    ...
    // 从blockManagerIdByExecutor map中移除executorId对应的BlocManagerInfo
    ...
    while (iterator.hasNext) {
      // 清空BockManagerInfo内部的block的BlockStatus信息
  ...
  private def register(id: BlockManagerId, maxMemSize: Long, slaveActor: ActorRef) {
    ...
    if (!blockManagerInfo.contains(id)) {
      ...
      // 那么同步的blockManagerIdByExectuor map里，也必须没有
      ...
      blockManagerIdByExecutor.get(id.executorId) match {
        case Some(oldId) =>
          ...
          // 从内存中，移除到executorId相关的blockManagerInfo
      ...
      // 并王blockManagerInfo map中，保存一份blockManagerId到BlockManagerInfo的映射
  ...
  private def updateBlockInfo(
    ...
    // 很为如果将StorageLevel设置成带着_2的这种，那么就需要将block replicate一份，放到其他BlockManager上
...
private[spark] class BlockManagerInfo(
  ...
  def updateBlockInfo(
    ...
    if (_blocks.containsKey(blockId)) {
      ...
      // 判断如果storageLevel是使用内存，那么就给剩余内存数量加上当前的内存量
