...
class SQLContext(@transient val sparkContext: SparkContext)
   ...
   * Unresolved LogicalPlan，去调用Analyzer的apply()方法，来将Unresolved LogicalPlan来生成一个
  ...
  def sql(sqlText: String): DataFrame = {
    ...
    if (conf.dialect == "sql") {
      ...
      // Spark SQL的lazy特性，我们在这里通过源码来深度剖析一下
      ...
      // 在用户拿到封装了Unresolved LogicalPlan的DataFrame之后
      // 然后可以，执行一些show()、select().show()、groupBy().show()这样的操作
      ...
      // 这里，我们首先看第一步
      // parseSql()方法，传入SQL语句，调用SqlParser解析SQL，获取Unresolved LogicalPlan
      ...
      // 那么解析了SQl，拿到Unresovled LogicalPlan之后，会封装一个DataFrame，返回给用户
      // 用户此时就可以开始用DataFrame执行各种操作了
  ...
  protected[sql] class QueryExecution(val logical: LogicalPlan) {
    ...
    // Analyzer的apply()方法执行结束后
    // 得到一个Resolved LogicalPlan
    ...
    // 最后一步，调用SparkPlan（封装了PysicalPlan的SparkLan）的execute()方法
    // execute()方法，实际上就会去执行物理执行计划
    // 最后一个重要的点，就是，execute()方法，返回的是什么？？
    // RDD[Row]，就是一个元素类型为Row的RDD
